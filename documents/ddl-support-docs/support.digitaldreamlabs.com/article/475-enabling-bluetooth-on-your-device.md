-   [Contact](#)

# Enabling Bluetooth On Your Device

[](javascript:window.print() "Print this article")

The Vector Robot app uses Bluetooth technology to communicate between Vector and your mobile device. Bluetooth is required to be enabled on your mobile device or tablet to connect to Vector. Without Bluetooth enabled, you won't be able to set Vector up or connect with him using your device after setup.

**Please Note: You will not be able to see Vector in the Bluetooth menu of your device. You may see his Cubes, but they cannot be paired to. The Vector Robot app handles all Bluetooth communication and pairing with Vector through the Bluetooth Settings screen is not necessary.**

------------------------------------------------------------------------

### Instructions for iPhone

From the Home screen:

1.  Scroll through your Home screens to find the "Settings" app, then click on it to enter the Settings menu for your device.

2.  Find "Bluetooth" in the menu. Click on "Bluetooth."

3.  Drag the slider to the right so that the slider is green.

    ![Enabling Bluetooth on iOS 15](//d33v4339jhl8k0.cloudfront.net/docs/assets/5e3f0b1e2c7d3a7e9ae777f5/images/61e5926b8200bc052eb7f6a8/file-UOCrEHNQ8g.jpg)

------------------------------------------------------------------------

### Instructions for Android

From the Home screen:

1.  Find the Settings app (this will vary by software setup and phone manufacturer):
    1.  If you have an App Drawer:
        1.  Locate the App Drawer button at the bottom of the screen.
        2.  Click the App Drawer icon.
        3.  Rotate through App Drawer screens until you find the Settings app.
        4.  Click the Settings icon to enter the Settings app.
    2.  If you do not have an App Drawer:
        1.   Rotate through your Home screens to find the Settings icon.
        2.  Click on the Settings icon to enter the Settings menu.
2.  Find the "Bluetooth" menu. This could be under its own dedicated section, or could be under the "Networks" menu.
3.  Drag the "Bluetooth" slider to the "On" position.

Did this answer your question? ![](data:image/svg+xml;base64,) Thanks for the feedback There was a problem submitting your feedback. Please try again later.

Yes ![](data:image/svg+xml;base64,)

No ![](data:image/svg+xml;base64,)

Still need help? [Contact Us](#) [Contact Us](#)

Last updated on January 17, 2022

Toggle Search

### Categories

-   [Customer Care Information Screen ](/category/524-customer-care-information-screen)
-   [Frequently Asked Questions ](/category/15-frequently-asked-questions)
-   [Vector Membership ](/category/486-vector-membership)
-   [Setting Vector Up ](/category/491-setting-vector-up)
-   [Software Updates ](/category/483-software-updates)
-   [Interacting with Vector ](/category/74-living-with-vector)
-   [Charging & Power ](/category/489-charging-power)
-   [Vector's Cube ](/category/482-vectors-cube)
-   [Documents & Manuals ](/category/484-documents-manuals)
-   [Reset Vector ](/category/487-reset-vector)
-   [Open Source Kit for Robotics ](/category/343-oskr)
-   [Product Information ](/category/67-product-information)
-   [SDK Information ](/category/34-sdk-information)
-   [Connection Troubleshooting ](/category/481-connection-troubleshooting)
-   [Error Troubleshooting ](/category/488-error-troubleshooting)
-   [Speech Recognition Troubleshooting ](/category/490-speech-recognition-troubleshooting)
-   [Sound Troubleshooting ](/category/485-sound-troubleshooting)
-   [Technical Troubleshooting ](/category/16-troubleshooting)

No results found

© [Digital Dream Labs](https://support.digitaldreamlabs.com/) 2023. Powered by [Help Scout](https://www.helpscout.com/knowledge-base/?utm_source=docs&utm_medium=footerlink&utm_campaign=Docs+Branding)
